 ...
 <?php include ("include/navigation.inc.php");?> 
  ...
  while($row=mysqli_fetch_array($Query)){$i++;?>  
			  ...
			  echo number_format($totalResult['total'],2);?></td>
			      ...
			      <td><?php 		$total_bonus =mysqli_query($con,"select sum(`level1`)as total from `tbl_bonus` where level1 > 0 AND userid = '".$row["id"]."'");
	$totalResult=mysqli_fetch_array($total_bonus);
echo number_format($totalResult['total'],1); ?></td>


			  <td><?php   	$total_bonus1 =mysqli_query($con,"select sum(`level2`)as total from `tbl_bonus` where level2 > 0 AND userid = '".$row["id"]."'");
	$totalResult=mysqli_fetch_array($total_bonus1);
echo  number_format($totalResult['total'],1); ?></td>
...
echo number_format($totalResult['total'],1);?></td>
      ...
      <a href="javascript:void(0);" onClick="UnRespond(<?php echo $row['id']; ?>)" class="update-person" style="color:#f00; font-size:16px;" data-toggle="tooltip" title="Unpublish"><i class="fa fa-square-o"></i></a>
       ...
       &nbsp;  <a href="user_game_history.php?user=<?php echo $tableuserid?>"  class="update-person" style="color:#E9D94F; font-size:16px;" data-toggle="tooltip" title="History"><i class="fa fa-history"></i></a>    
